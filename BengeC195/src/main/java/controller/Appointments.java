...
public class Appointments implements Initializable {
    ...
    // Gather language information and change properties based on data.
    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (week.isSelected()){
            apptTable.setItems(AppointmentDB.getAppointments(CID, 14));
        }
        if (month.isSelected()){
    ...
    public void handleEditAppointments(ActionEvent actionEvent) throws IOException {
        if (AID ==0) {
            ...
            appointment = apptTable.getSelectionModel().getSelectedItem();
            System.out.println(apptTable.getSelectionModel().getSelectedCells());
            Stage stage = (Stage) ((Button) actionEvent.getSource()).getScene().getWindow();
    ...
    public void handleDeleteAppointment(ActionEvent actionEvent) {
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Are you sure?");
        alert.setContentText("Deleting an appointment cannot be undone.");
        Optional<ButtonType> choice = alert.showAndWait();
        if (choice.isPresent() && choice.get() == ButtonType.OK) {
            if (AID ==0) {
                Alert eAlert = new Alert(Alert.AlertType.NONE);
